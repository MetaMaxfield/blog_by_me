...
- Удаление неиспользуемых медиафайлов с помощью сигналов;
...
- Мультиязычность моделей и шаблонов страниц (Русский, Английский);
- Защита от интернет-ботов (ReCAPTCHA v3);
- Оптимизированные запросы к базе данных;
- Пользовательские команды;
- Логгирование уровней WARNING и ERROR в отдельный файл;
- Кэширование.
...
├── logs                      # Папка с файлами для логгирования
...
4. Определяем pre-commit хуки

    ```
    pre-commit install
    ```

5. Создаем базу данных в PostgreSQL

6. Добавляем проект в приложения Google аккаунта и сохраняем пароль приложения

7. Регистрируем сайт на Google ReCAPTCHA, сохраняем ключ сайта и секретный ключ

8. Устанавливаем и запускаем Memcached

9. Сгенерируем и сохраним SECRET_KEY для проекта
...
10. Создаем файл ``` .env ``` в папке с проектом и добавляем переменные окружения
...
12. Создаем суперпользователя

    ```
    python manage.py createsuperuser
    ```

13. Выполняем необходимые пользовательские команды

    - Создаем расширенную плоскую страницу "О нас"
        ```
    ...
    - Создаем группы пользователей
        ...
        ```
    - Создаем оценки рейтинга
        ```
        python manage.py create_mark_models
        ```
...
3. [ChatGPT-4o от OpenAI](https://chatgpt.com/)
4. [Готовый HTML шаблон](http://html-template.ru/blogi/item/289-modnyj-i-sovremennyj-blog)