    ...
    class Meta:
        ...
        widgets = {
            ...
            'email': forms.EmailInput(
                attrs={'class': 'span10', 'data-required': 'true', 'data-type': 'email'}
            ),
            'text': forms.Textarea(
                attrs={'class': 'span10', 'data-trigger': 'keyup', 'id': 'contactcomment'}
            )
        }
...
class RatingForm(forms.ModelForm):
    ...
    mark = forms.ModelChoiceField(
        queryset=Mark.objects.all().order_by('-value'), widget=forms.RadioSelect(),
        empty_label=None
    )
